...
Discovery is **one** walk — `\( -type f <gates> -o -type l \)` — and each entry's class is refined afterwards with `[ -h ]`. That is the shape portable collectors converge on (`unix_collector` walks the same union once and refines per entry; UAC uses `-type` only as a filter and re-tests with `[ -f ] || [ -h ]`), because no `find` reports *which* arm matched in a portable way: `-printf` is GNU-only — FreeBSD's is a documented stub with no `%y`, macOS has none — and busybox has neither `-printf` nor `-ls`. Splitting discovery into a `-type l` walk and a `-type f` walk was tried and reverted: two walks are not simultaneous, so an entry that changed type between them matched neither and left the inventory with no counter and no diagnostic, and a count-based check added to detect that reported ordinary file *creation* as a lost entry and forced exit 4. `[ -h ]` answering false is **not** taken as proof of a regular file: it also answers false when the `lstat` itself was refused, which is what a readable-but-not-searchable parent does. Such an entry is carried as a regular file and then fails `[ -f ]` in the upload pass, where it is booked as **unreadable** — a named failure and exit 4 — rather than having a type invented for it or being called churn. What is given up is knowing it was specifically a symlink: the discovered breakdown counts it among the regular files and `links_seen=` does not include it. That is a reporting limitation, not an impossibility — GNU `find` can still answer `-type l` for such an entry from the directory entry, and a targeted per-entry probe could recover the class. The collector does not pay a `find` fork per denied entry to do so; the entry is counted exactly once and reported as a named failure either way, which is the fact that matters for coverage. An entry whose type changes between discovery and upload — a symlink replaced by a regular file, or the reverse — is not collected and counts as failed (`vanished`), because find's size and age predicates were never applied to the new object.
